...
resource "aws_network_acl" "private" {
  ...
  # Allow inbound traffic from VPC (for App Mesh, service discovery, VPC endpoints)
  ingress {
    protocol   = "-1"
    rule_no    = 100
    action     = "allow"
  ...
  # Allow all outbound traffic within VPC (for App Mesh, service discovery, VPC endpoints)
  egress {
    protocol   = "-1"
    rule_no    = 130
    action     = "allow"
